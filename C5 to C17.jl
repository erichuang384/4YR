...
for i in 1:length(models)
    viscosity[:,i] = IB_viscosity_const.(models[i],P,T_range[:]) 
end

for i in 1:length(models)
    viscosity_IB[:,i] = IB_viscosity_test.(models[i],P,T_range[:]) 
end
...
for i in 1:length(models)
    ...
    n_calculated = IB_viscosity_const.(models[i],P_experimental,T_experimental) 
