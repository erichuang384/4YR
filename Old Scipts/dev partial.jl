...
function gamma_exponent(model::EoSModel, P, T)
    vol = Clapeyron.volume(model, P, T)
    hess_p,grad_p,pval = ∂2p(model,vol, T)
    ∂p_∂T = grad_p[2] 
    ∂p_∂T_res = ∂p_∂T - Rgas()/vol
    d2f_dT2_res = ∂²f∂T²_res(model,vol, T,1.0)
    Cv_ex_T = -T*d2f_dT2_res
    gamma_ex = vol * ∂p_∂T_res / Cv_ex_T
    return gamma_ex
end
#= model =  SAFTgammaMie(["Hexane"])
vol = Clapeyron.volume(model,1e5,300.0)
Clapeyron.VT_isothermal_compressibility(model, vol, 300.0)

T = 300.0

VT_isothermal_compressibility(model, vol, 300.0)

hess_p,grad_p,pval = ∂2p(model,vol,300.0, z = StaticArrays.SA[1.0])
 =#
function ∂f∂V(model,V,T,z::AbstractVector)
    f(∂V) = Clapeyron.a_res(model,∂V,T,z)
    ∂aᵣ∂V = Solvers.derivative(f,V)
    sum(z)*Rgas(model)*T*(∂aᵣ∂V - 1/V)
end

#= hess_p,grad_p,pval = ∂2p(model,vol, T)
#hess_p_ideal,grad_p_ideal,pval_ideal = ∂2p_ideal(model,vol, T)
∂p_∂T = grad_p[2] 
∂p_∂T_res = ∂p_∂T - Rgas()/vol

 ∂p_ex_∂T(model,vol, T)
 =#
function ∂2f(model,V,T)
    f(_V,_T) = eos(model,_V,_T)
    _f,_∂f,_∂2f = Solvers.∂2(f,V,T)
    return (_∂2f,_∂f,_f)
end


#= d2f_dT2 = ∂²f∂T²(model,vol, T,1.0)
d2f_dT2_res = ∂²f∂T²_res(model,vol, T,1.0)

Cv_ex_T = -T*d2f_dT2_res
gamma_exp = vol * ∂p_∂T_res / Cv_ex_T
crit_point = crit_pure(model)

gamma_exponent(model,1e5,300.0)
gamma_exponent(model,crit_point[2],crit_point[1]) =#





# Ideal gas dp/dV is just nR/V but since V is molar, R/V
#∂p_∂V_ideal = Rgas()/vol

function ∂2p(model,V,T)
    ideal_pres = Rgas() * T/V
    f(_V,_T) = pressure(model,_V,_T)
    #f(V,T) = pressure(model,V,T)
    _f,_∂f,_∂2f = Solvers.∂2(f,V,T)
    return (_∂2f,_∂f,_f)
end
function ∂p_ex_∂T(model,V,T)
    hess_p,grad_p,pval = ∂2p(model,V,T)
    ∂p_∂T = grad_p[2] 
    ∂p_∂T_ideal = ∂p_∂T - Rgas()/V
    return ∂p_∂T_ideal 
end
...
function ∂²f∂T²_res(model,V,T,z)
    A(x) = Clapeyron.eos_res(model,V,x,z)
    ∂A∂T(x) = Solvers.derivative(A,x)
    ∂²A∂T²(x) = Solvers.derivative(∂A∂T,x)
    return ∂²A∂T²(T)
end
